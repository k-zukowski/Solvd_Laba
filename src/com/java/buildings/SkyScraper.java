...
public class SkyScraper extends Building implements IBuildingTypeFunctionality {
    ...
    public SkyScraper(float height, float width, float length, int storyCount, boolean isBasement, boolean isUndergroundCarpark, ArrayList<Architect> architects, ArrayList<Builder> builders, SkyScraperType type, int earthShakeProtection) {
        ...
        try {
            if (earthShakeProtection < 0 || earthShakeProtection > 10) {
                throw new WrongEarthShakeProtectionException(this.getClass().getName(), hashCode(), getEarthShakeProtection());
            }
