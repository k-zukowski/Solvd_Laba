...
public class ClientRepresentative extends Person {
    ...
    public ClientRepresentative(String name, String surname, LocalDate birthDay, String email) throws ValidateEmailException {
    ...
    private void validateEmailException() throws ValidateEmailException {
        ...
        if (isLatin) {
            if (!patternMatches(getEmail(), "^(?=.{1,64}@)[\\p{L}0-9_-]+(\\.[\\p{L}0-9_-]+)*@"
                ...
                throw new ValidateEmailException(this.getClass().getName(), hashCode(), isLatin, String.valueOf(getEmail()));
            }
        } else {
